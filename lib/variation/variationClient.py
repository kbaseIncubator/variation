...
class variation(object):
    ...
    def variation_example_method_async(self, input, json_rpc_context = None):
        if json_rpc_context and type(json_rpc_context) is not dict:
            raise ValueError('Method variation_example_method: argument json_rpc_context is not type dict as required.')
        return self._call('variation.variation_example_method_async',
                          [input], json_rpc_context)[0]

    def variation_example_method_check(self, job_id):
        resp = self._call('variation.variation_example_method_check', [job_id])
        return resp[0]

    def variation_example_method(self, input, json_rpc_context = None):
        job_id = self.variation_example_method_async(input, json_rpc_context)
        while True:
            time.sleep(self.async_job_check_time)
            job_state = self.variation_example_method_check(job_id)
